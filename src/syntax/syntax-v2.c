...
bool StatementList(){
    c++;
    if(getFullTokenCount() + 1 < c){
        error("syntax analyzer validation failed\nbuffer overflowed");
        return false;
    } 
...
bool Statement(){
    ...
    if(v) {
        printf("t(%d) ✅ Variable declaration\n" , getTokenNumber());
        return true;
    }else{
